...
/* ── Nota interna por cita ──────────────────────────────────── */

function InternalNoteRow({ appointmentId, initialNote }: { appointmentId: string; initialNote?: string | null }) {
  ...
  async function save() {
    ...
    setSaved(false);
    try {
      await clientApi(`/api/assistants/appointments/${appointmentId}`, {
        method: "PATCH",
        body: JSON.stringify({ assistantNotes: note })
      });
      setSaved(true);
      setTimeout(() => setSaved(false), 3000);
    } catch {
      // silencioso — el médico puede reintentar
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="mt-2 border-t border-slate-100 pt-2">
      ...
      >
        ...
        {note ? "Ver/editar nota interna" : "Agregar nota interna"}
      </button>
      {open && (
        <div className="mt-2 flex flex-col gap-2">
          <textarea
            ...
            placeholder="Ej. Paciente solicita factura, llega en silla de ruedas, paga con tarjeta..."
            rows={2}
            className="w-full resize-none rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-[#071726] outline-none transition focus:border-[#1e9bd4]/40 focus:ring-2 focus:ring-[#1e9bd4]/10"
          />
          <div className="flex items-center gap-2">
            <button
              onClick={save}
              disabled={saving}
              className="flex items-center gap-1.5 rounded-full bg-[#071726] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-[#0d2638] disabled:opacity-50"
            >
              {saving ? <Loader2 className="h-3 w-3 animate-spin" /> : null}
              Guardar nota
            </button>
            {saved && <span className="text-xs font-semibold text-emerald-600">✓ Guardada</span>}
            <p className="text-xs text-slate-400">Solo visible para el equipo médico.</p>
          </div>
        </div>
      )}
    </div>
              ...
              {todayAppointments.map((appt) => {
                ...
                return (
                  ...
                  >
                    ...
                    <InternalNoteRow appointmentId={appt.id} initialNote={appt.assistantNotes} />
                  </div>
                );
              })}
            </div>
